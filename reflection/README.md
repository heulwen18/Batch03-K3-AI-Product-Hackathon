# Reflection — mỗi thành viên 1 file

Chấm riêng theo rubric của khoá. Mỗi người copy `_template.md` thành `<tên>.md` và tự viết —
**không nhờ AI viết hộ toàn bộ** (vibe-coding rule áp cho cả reflection: phải giải thích được
những gì mình viết).

| File | Người | Phần phụ trách |
|---|---|---|
| `manh.md` | Mai Anh | Slide + Evidence/mining — xử lý dữ liệu, phân tích chatlog |
| `hoa-mai.md` | Hoa Mai | Frontend Streamlit + deploy |
| `phuong.md` | Phương | Pipeline phân loại + tích hợp |
| `linh.md` | Linh | Frontend + Form, kịch bản |
| `phuong-test.md` | Phượng | Kiểm thử (golden set, eval) + tối ưu |
